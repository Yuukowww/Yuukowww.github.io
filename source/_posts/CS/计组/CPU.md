...
- 超标量流水线技术(动态多发射) -- 一个周期并发多条指令，通常以顺序执行(现代计算机支持乱序执行)，控制较为复杂
...
- 直接编码方式 -- 每一位直接对应一个微指令
- 字段直接编码方式 -- 分为若干互斥段编码， 互斥的微指令归入相同的互斥段，相容的微指令放入不同的互斥段。互斥段之间的相容进指令进行静态多发射类似的并发。每一个字段内还保留一个全置0的状态，因此有效的互斥微指令只有 $2^n-1$ 条
