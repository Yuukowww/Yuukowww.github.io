...
适合项目就在`C:\Users\...\code`、需要 PowerShell / Visual Studio / Windows SDK 等原生工具链的场景。官方现在支持 Windows 原生运行，默认会使用 Windows sandbox；如果机器策略允许，优先用`elevated` sandbox。
...
如果 Codex 提示 sandbox 读不到某个目录，在 Codex 里给只读权限：
```text
...
适合项目本来就在 Linux 工具链里，或者需要 bash、Docker、Linux 包管理器与更接近服务器的环境。仓库建议放在 WSL 的`~/code`里，不要放在`/mnt/c/...`，速度和权限问题会少很多。

Codex CLI
```shell
# PowerShell管理员窗口中安装并进入WSL
wsl --install
wsl

# WSL内安装Node.js与Codex
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/master/install.sh | bash
# 重新打开一个WSL窗口，或执行 source ~/.bashrc / source ~/.zshrc
nvm install 22
...
WSL建议用WSL2。Codex `0.115`之后 Linux sandbox 迁移到`bubblewrap`，WSL1不再适合。开发时可以从WSL内部打开VSCode：
```shell
mkdir -p ~/code
