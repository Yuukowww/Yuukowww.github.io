---
title: 平稳序列
description: 时间序列的基本概念
categories: 时间序列分析
date: 2025-06-20
tikzjax: true
math: true
cover: picture/asuna1.jpg
---
...
$$
\begin{aligned}
\mathrm{Cov}(X,Y)&=\mathbb{E}(\left[ X-\mu_X\right]\left[ Y-\mu _Y\right])\\
&= \mathbb{E}(XY-X\mu_Y-Y\mu_X+\mu_X\mu_Y)\\
&= \mathbb{E}(XY)-\mu_Y\mathbb{E}(X)-\mu_X\mathbb{E}(X)+\mathbb{E}(X)\mathbb{E}(Y)\\
...
\alpha^T\Gamma_n\alpha&=\sum_{i,j}a_ia_j\gamma_{i-j}\\
		&=\sum_{i,j}a_ia_j\mathbb{E}(\left[X_i-\mu\right]\left[X_j-\mu\right])\\
		&=\mathbb{E}\left(\sum_{i,j}a_ia_j\left(X_i-\mu\right)(X_j-\mu)\right)\\
		&=\mathbb{E}\left(\sum_{i}a_i\left(X_i-\mu\right)\sum_ja_j(X_j-\mu)\right)\\
		&=\mathbb{E}\left(\left[\sum_ia_i(X_i-\mu)\right]^2\right)\\
...
- $(3)$ 有界性 $|\gamma_k|\leq |\gamma_0|<\infty$
	...
	$$
		\begin{aligned}
		|\gamma_k|&=\left|\mathbb{E}\left[\left(X_u-\mu\right)(X_v-\mu)\right]\right|\\
		&\leq \sqrt{\mathbb{E}[(X_u-\mu)]^2\,\mathbb{E}[(X_v-\mu)^2]}\\
...
- 定义
	...
	$$
	\forall s,t \in \mathbb{N},\,\mathbb{E}(\varepsilon_t)=\mu ,\,\mathrm{Cov}(\varepsilon_s,\varepsilon_t)=\delta_{st}\sigma^2
	$$
...
$$
\begin{align*}
\mathrm{Cov}(X_t,X_s)&=\mathbb{E}(X_t,X_s)\\
...
&=\frac{1}{\sqrt{\pi}}e^{-\tfrac{(s-t)^2}{4}}\,\mathbb{E}({\small N(-\frac{s+t}{2},\frac{1}{\sqrt{2}})})\\
