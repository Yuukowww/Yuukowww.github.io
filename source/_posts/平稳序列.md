...
$$
\begin{aligned}
\mathrm{Cov}(X,Y)&=\mathrm{E}(\left[ X-\mu_X\right]\left[ Y-\mu _Y\right])\\
&= \mathrm{E}(XY-X\mu_Y-Y\mu_X+\mu_X\mu_Y)\\
&= \mathrm{E}(XY)-\mu_Y\mathrm{E}(X)-\mu_X\mathrm{E}(X)+\mathrm{E}(X)\mathrm{E}(Y)\\
